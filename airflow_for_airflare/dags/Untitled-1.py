...
# %%
    ...
    def transform(info):  # Функция оставляет только нужные нам данные
        ...
        cur_date = pd.Series([datetime.date(datetime.today())] * len(
            df), name='date_of_extraction').astype('str')  # Создает серию хранящую текущую дату
        ...
        path = upload_to_s3(
            content, bucket=path['bucket'], key=f"DataFrames/df_{rout['origin']}_{rout['destination']}.json")  # Загружает в S3
